...
// Omit the .xcdatamodeld extension on the model file name
// Save to disk
VOKCoreDataManager.shared.setResource("VOKCoreDataModel",
                                      database: "VOKCoreDataModel.sqlite")

// Or, create an in-memory store:
VOKCoreDataManager.shared.setResource("VOKCoreDataModel", database: nil)
```
...
**Objective-C**
Note that this example makes use of the `VOKKeyForInstanceOf` macro from [VOKUtilities/VOKKeyPathHelper](https://github.com/vokal/VOKUtilities#vokkeypathhelper), which is a dependency for Vokoder. These macros are not available to Swift.
```objective-c
...
**Swift**
Note that the [VOKUtilities/VOKKeyPathHelper](https://github.com/vokal/VOKUtilities#vokkeypathhelper) macros can't be used in Swift, so this example just uses strings for the Core Data keys. It's an error-prone approach, since it lacks compile-time checks. As such, we recommend using something like the attribute enums that are generated by [mogenerator](https://github.com/rentzsch/mogenerator), to ensure that there aren't typos in the `coreDataKey` parameter. `VOKManagedObjectMap` includes an initializer that takes such enums, as `coreDataKeyEnum`.
```swift
// A date formatter will enable Vokoder to turn strings into NSDates
let dateFormatter = DateFormatter.someCustomDateFormatter()
// A number formatter will do the same, turning strings into NSNumbers
let numberFormatter = NumberFormatter()
let maps: [VOKManagedObjectMap] = [
    VOKManagedObjectMap(foreignKeyPath: "ticket_number", coreDataKey: "ticketNumber"),
    VOKManagedObjectMap(foreignKeyPath: "first_name", coreDataKey: "firstName"),
    VOKManagedObjectMap(foreignKeyPath: "last_name", coreDataKey: "lastName"),
    VOKManagedObjectMap(foreignKeyPath: "ss_num", coreDataKey: "socialSecurityNumber"),
    VOKManagedObjectMap(foreignKeyPath: "salary", coreDataKey: "salary", numberFormatter: numberFormatter),
    VOKManagedObjectMap(foreignKeyPath: "dob", coreDataKey: "dateOfBirth", dateFormatter: dateFormatter)
]
// The unique key is an NSString to uniquely identify local entities. If nil, each import can create duplicate objects.
let mapper = VOKManagedObjectMapper(uniqueKey: "ticketNumber", andMaps: maps)
// By default, missing parameters and null parameters in the import data will nil out an attribute's value.
// With ignoreNullValueOverwrites set to YES, the maps will leave existing attributes alone unless new data is provided.
mapper.ignoreNullValueOverwrites = true
// By default, Vokoder will complain about every single parameter that can't be set.
// With ignoreOptionalNullValues set to YES, Vokoder will not warn about mismatched classes or null/nil values.
mapper.ignoreOptionalNullValues = true
// Set the mapper and Vokoder will handle the rest.
VOKCoreDataManager.shared.setObjectMapper(mapper, for: VOKPerson.self)
```
...
```swift
// Asynchronous import
TrainLine.vok_addWithArray(inBackground: importArray) { arrayOfManagedObjects in
    // This completion block runs on the main queue
    let trainLines = arrayOfManagedObjects as? [TrainLine] ?? []
}

// Synchronous import, with typed return
let newTrainLines = TrainLine.vok_import(importArray) // Array of TrainLine objects
```

For more control over background operations, the `VOKCoreDataManager` shared instance offers more generic methods. Vokoder will handle queues and provide a temporary context without automatically importing or returning anything. 
...
**Swift**
```swift
open func write(toTemporaryContext writeBlock: @escaping Vokoder.VOKWriteBlock, completion: (() -> Swift.Void)? = nil)
```
...
[backgroundContext performBlock:^{
    TrainLine *trainLine = [TrainLine vok_newInstanceWithContext:backgroundContext];
    trainLine.name = @"Merchandise Mart";
...
**Swift**
```swift
let backgroundContext = VOKCoreDataManager.shared.temporaryContext()
backgroundContext.perform {
    let trainLine = TrainLine.vok_newInstance(with: backgroundContext)
    trainLine.name = "Merchandise Mart"
    VOKCoreDataManager.shared.saveAndMerge(withMainContext: backgroundContext)
}
```
...
**Swift**
```swift
let person = TrainLine.vok_newInstance()
person.firstName = "Rohan"
person.lastName = "Panchal"
VOKCoreDataManager.shared.saveMainContextAndWait()
```

###Querying Records
Like the example above, the Objective-C examples below make use of the `VOKKeyForInstanceOf` macro from [VOKUtilities/VOKKeyPathHelper](https://github.com/vokal/VOKUtilities#vokkeypathhelper) for the keys in these calls to ensure that typos like `@"lsatName"` don't occur.
...
//Saves the main context asynchronously
VOKCoreDataManager.shared.saveMainContext()

//Save a temp context and merge changes to the main context asynchronously
VOKCoreDataManager.shared.saveAndMerge(withMainContext: tempContext)

//Save a temp context and merge changes to the main context synchronously
VOKCoreDataManager.shared.saveAndMerge(withMainContextAndWait: tempContext)
```
